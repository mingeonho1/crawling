    ...
    for row in keyword:
        try:
            ...
            for content in contents:
                ...
                if not row[3].replace("영화", "") in text + title + user and not row[2].replace("(변형 키워드)",
                                                                                              "") in text + title + user:
                    print()
                    print("키워드 키메인 체크!!\n"
                          "넘어감!!")
...
def main():
    cps = cp_list()
    for cp in cps:
        db_result_list = crawling(cp[1])

        if any(db_result_list) == True:
            updateOspState(0, 'YOUTUBE', cp[1])
        else:
            updateOspState(1, 'YOUTUBE', cp[1])

        updateOspEnddate(datetime.datetime.now().strftime('%Y-%m-%d'), 'YOUTUBE', cp[1])
